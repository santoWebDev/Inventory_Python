...
class InventoryService:
    ...
    def _change(db, user_id, product_id, new_stock, quantity, kind, reason):
        ...
        if product.status != "active":
            raise AppException("Cannot update inactive product", 400)
    ...
    @staticmethod
    def stock_in(db, user_id, data):
        return InventoryService._change(
            db,
            user_id,
            data.productId,
            entity_or_404(db, Product, data.productId, "Product").stock + data.quantity,
            data.quantity,
            "IN",
            data.reason,
        )
    ...
    def stock_out(db, user_id, data):
        ...
        if p.stock < data.quantity:
            raise AppException("Insufficient stock", 400)
        return InventoryService._change(
            db,
            user_id,
            p.id,
