...
class ProductService:
    ...
    def _validate_refs(db,category_id,supplier_id):
        if not db.get(Category,category_id): raise AppException("Category not found",404)
        if not db.get(Supplier,supplier_id): raise AppException("Supplier not found",404)
    @staticmethod
    def create(db,data):
    ...
    def get_all(db,search=None,category=None,status=None,page=1,limit=10):
        ...
        if category: q=q.where(Product.category_id==int(category))
    ...
    @staticmethod
    def get_by_id(db,id): return product_payload(entity_or_404(db,Product,id,"Product"))
    @staticmethod
    def update(db, product_id, data):
        ...
        supplier_id = (
            ...
            else obj.supplier_id
        )
    
        # Validate only the final IDs
        ProductService._validate_refs(
            db,
            category_id,
            supplier_id
        )
    ...
    @staticmethod
    def delete(db,id):
        obj=entity_or_404(db,Product,id,"Product"); obj.status="inactive"; db.commit(); db.refresh(obj); return product_payload(obj)
