...
class SupplierService:
    ...
    def create(db, data):
        if db.scalar(
            ...
            raise AppException("Supplier name already exists", 409)
        if db.scalar(
            select(Supplier).where(
                func.lower(Supplier.email) == str(data.email).lower()
            )
        ):
            raise AppException("Supplier email already exists", 409)
        obj = Supplier(
    ...
    def get_all(db, search=None, status=None, page=1, limit=10):
        ...
        if search:
            q = q.where(
                ...
                | Supplier.number.ilike(f"%{search}%")
            )
        if status:
    ...
    @staticmethod
    def get_by_id(db, id):
        return entity_or_404(db, Supplier, id, "Supplier")
