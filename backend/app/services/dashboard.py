...
class DashboardService:
    ...
    def get(db):
        ...
        low = [
            p
            for p in products
            if p.status == "active" and p.stock > 0 and p.stock <= p.low_stock_threshold
        ]
        ...
        return {
            "products": {
                "total": len(products),
                "active": sum(p.status == "active" for p in products),
                "inactive": sum(p.status == "inactive" for p in products),
                "lowStock": len(low),
                "outOfStock": len(out),
            },
            "categories": {
            ...
            "orders": {
                ...
                "confirmed": sum(o.status == "confirmed" for o in orders),
                "shipped": sum(o.status == "shipped" for o in orders),
                "delivered": sum(o.status == "delivered" for o in orders),
                "cancelled": sum(o.status == "cancelled" for o in orders),
            },
            "sales": {"total": sum(o.total_amount for o in non_cancelled)},
            "recentOrders": orders[:5],
            "lowStockProducts": low[:10],
        }
