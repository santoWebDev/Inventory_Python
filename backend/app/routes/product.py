...
def get_all(
    ...
    items, total = ProductService.get_all(db, search, category, status, page, limit)
    return {
        "success": True,
        "count": total,
...
def create(data: ProductCreate, db: Session = Depends(get_db)):
    return {
        ...
        "data": ProductService.create(db, data),
    }


@router.get("/{id}", dependencies=[Depends(current_user)])
def get_one(id: int, db: Session = Depends(get_db)):
    return {"success": True, "data": ProductService.get_by_id(db, id)}
...
def update(id: int, data: ProductUpdate, db: Session = Depends(get_db)):
    return {
        ...
        "data": ProductService.update(db, id, data),
    }
...
def delete(id: int, db: Session = Depends(get_db)):
    return {
        ...
        "data": ProductService.delete(db, id),
    }
